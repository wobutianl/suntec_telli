...
public class ApiDemoActivity extends FragmentActivity {
    ...
    protected void onCreate(android.os.Bundle savedInstanceState) {
        ...
        mControlPanel.setOnEventListener(new ControlPanelFragment.OnEventListener() {
            ...
            public boolean onStartListening()
            {
                ...
                config.enableContacts(); // 启用通讯彿                config.enableVoicePower(Config.SHOW_VOL); // 音量反騆〿                if (Config.PLAY_START_SOUND) {
                    config.enableBeginSoundEffect(R.raw.bdspeech_recognition_start); // 设置识别弿ƾ提示響                }
                if (Config.PLAY_END_SOUND) 
                {
                    config.enableEndSoundEffect(R.raw.bdspeech_speech_end); // 设置识别结束提示響                }
                    config.setSampleRate(VoiceRecognitionConfig.SAMPLE_RATE_8K); // 设置采样玿霿Ơ与妁部音频一臿                // 下面发起识别
    ...
    class MyVoiceRecogListener implements VoiceClientStatusChangeListener {
        ...
        public void onClientStatusChange(int status, Object obj) 
        {
            switch (status) 
            {
            // 语音识别实际弿ƾ，这是真正开始识别的时间点，霿ܨ界面提示用户说话〿                case VoiceRecognitionClient.CLIENT_STATUS_START_RECORDING:
            	case VoiceRecognitionClient.CLIENT_STATUS_ERROR:
                    ...
                    break;
                case VoiceRecognitionClient.CLIENT_STATUS_SPEECH_START: // 棿Ջ到语音起炿                    mControlPanel.statusChange(ControlPanelFragment.STATUS_SPEECH_START);
                    break;
                // 已经棿Ջ到语音终点，等待网络返回
